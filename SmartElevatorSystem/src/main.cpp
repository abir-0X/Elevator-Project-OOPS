/*
 * Context for main.cpp
 *
 * REQUIREMENT: 
 *   Verifying Chapter 6 Complete Simulation. We need a continuous execution flow to tie all components together into an interactive application.
 * 
 * DESIGN: 
 *   A continuous command-line interface (CLI) menu that allows users to enter floor calls, make cabin calls, step the simulation, and catch any errors.
 * 
 * IMPLEMENTED: 
 *   Developed a console-based while loop with input validation, try-catch exception handling blocks, and step driving logic.
 * 
 * RESULT: 
...
int32_t main() {
    ...
    try {
        ...
        cout << "Fatal System Error: " << e.what() << endl;
        return 1;
    }
